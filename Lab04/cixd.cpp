...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   //Check File
   ifstream inFile (header.filename, ifstream::binary);
   if (inFile.fail()) {
      outlog << "Unable to Open " << header.filename << ": " 
         << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
   ...
   outlog << "sending header " << header << endl;
   send_packet (client_sock, &header, sizeof header);
   send_packet (client_sock, write, inFile.gcount());
   outlog << "sent " << inFile.gcount() << " bytes" << endl;
   delete[] write;
}
